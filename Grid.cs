...
public class Grid <TCell>
{
    ...
    public TCell GetCell(Coordinates coordinates)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(coordinates.column), "Column index out of range");
        }
        if (coordinates.row < 0 || coordinates.row >= rows)
        {
            throw new ArgumentOutOfRangeException(nameof(coordinates.row), "Row index out of range");
        }
    ...
    public void Draw()
    {
        for (int column = 0; column < columns; column++)
        {
            for (int row = 0; row < rows; row++)
            {
                ...
                Raylib.DrawRectangleLines((int)cellPos.X, (int)cellPos.Y, cellSize, cellSize, Color.Black);
            }
        }
    }
